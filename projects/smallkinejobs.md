...
At its core, Small Kine Jobs is a platform designed to enable UH students to find short-term job opportunities on campus. Many students have schedules that are too packed to accommodate a conventional full-time/part-time on-campus job. Others are only at the university for a semester or two. These students can surely benefit from having access to short-term jobs that last for only a few hours to a few days.
...
## Development
I would say that my contributions to the project fell into two broad categories: writing code to implement a number of the complex features provided by the application, and coaching my teammates towards a standard means of committing new features to the master branch of the application repository.
